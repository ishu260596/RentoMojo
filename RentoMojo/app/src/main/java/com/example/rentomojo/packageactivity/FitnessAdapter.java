...
public class FitnessAdapter extends RecyclerView.Adapter<FitnessAdapter.ItemViewHolder>{

    List<PackageItem> packageItemList;

    public FitnessAdapter(List<PackageItem> packageItemList) {
        this.packageItemList = packageItemList;
    }

    @NonNull
    @Override
    public FitnessAdapter.ItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.package_item_layout, parent, false);
        return new FitnessAdapter.ItemViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull FitnessAdapter.ItemViewHolder holder, int position) {
