...
public class AppliancesAdapter extends RecyclerView.Adapter<AppliancesAdapter.ItemViewHolder>{

    List<PackageItem> packageItemList;

    public AppliancesAdapter(List<PackageItem> packageItemList) {
        this.packageItemList = packageItemList;
    }

    @NonNull
    @Override
    public AppliancesAdapter.ItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.package_item_layout, parent, false);
        return new AppliancesAdapter.ItemViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull AppliancesAdapter.ItemViewHolder holder, int position) {
